...
void arcane( player_t* p )
{
  ...
  default_->add_action( "use_item,name=spymasters_web,if=(prev_gcd.1.arcane_surge|prev_gcd.1.evocation|(buff.ethereal_powerlink.up&buff.siphon_storm.remains&talent.spellfire_spheres))&(fight_remains<80|target.health.pct<35|!talent.arcane_bombardment|(buff.spymasters_report.stack=40&fight_remains>240))|fight_remains<20" );
  ...
  default_->add_action( "use_item,name=treacherous_transmitter,if=(cooldown.evocation.remains<7&cooldown.evocation.remains)|fight_remains<20" );
...
void frost( player_t* p )
{
  ...
  actions.precombat_->add_action( "flask" );
  actions.precombat_->add_action( "food" );
  actions.precombat_->add_action( "augmentation" );
  actions.precombat_->add_action( "arcane_intellect" );
  actions.precombat_->add_action( "snapshot_stats" );
  actions.precombat_->add_action( "variable,name=st_aoebuild,value=!talent.deep_shatter&!talent.cold_front" );
  actions.precombat_->add_action( "variable,name=st_ff,value=talent.frostfire_bolt" );
  actions.precombat_->add_action( "variable,name=time_to_coc,value=10*talent.frostfire_bolt" );
  actions.precombat_->add_action( "blizzard,if=active_enemies>=2&talent.ice_caller&!talent.fractured_frost|active_enemies>=4" );     
  actions.precombat_->add_action( "frostbolt,if=active_enemies<=3" );
    
  actions_->add_action( "counterspell" );
  actions_->add_action( "call_action_list,name=cds" );
  actions_->add_action( "run_action_list,name=aoe,if=active_enemies>=7|active_enemies>=3&!talent.frostfire_bolt|active_enemies>=4" );
  actions_->add_action( "run_action_list,name=cleave_ff,if=active_enemies>=2&active_enemies<=3&talent.frostfire_bolt" );
  actions_->add_action( "run_action_list,name=cleave_ss,if=active_enemies>=2" );
  actions_->add_action( "run_action_list,name=st_aoebuild,if=variable.st_aoebuild" );
  actions_->add_action( "run_action_list,name=st_ff,if=variable.st_ff" );
  actions_->add_action( "run_action_list,name=st_ss" );
    
  actions.cds_->add_action( "use_item,name=imperfect_ascendancy_serum,if=buff.icy_veins.remains>19|fight_remains<25" );
  actions.cds_->add_action( "use_item,name=spymasters_web,if=fight_remains<25|(fight_remains<100|buff.spymasters_report.stack>35)&(!talent.deaths_chill&buff.icy_veins.remains>19|talent.deaths_chill&buff.icy_veins.remains>15&buff.icy_veins.remains<20)" );
  actions.cds_->add_action( "potion,if=prev_off_gcd.icy_veins|fight_remains<60" );
  actions.cds_->add_action( "flurry,if=time=0&active_enemies<=2" );
  actions.cds_->add_action( "icy_veins" );
  actions.cds_->add_action( "use_items" );
  actions.cds_->add_action( "invoke_external_buff,name=power_infusion,if=buff.power_infusion.down" );
  actions.cds_->add_action( "invoke_external_buff,name=blessing_of_summer,if=buff.blessing_of_summer.down" );
  actions.cds_->add_action( "blood_fury" );
  actions.cds_->add_action( "berserking" );
  actions.cds_->add_action( "lights_judgment" );
  actions.cds_->add_action( "fireblood" );
  actions.cds_->add_action( "ancestral_call" );
    
  actions.aoe_->add_action( "cone_of_cold,if=talent.coldest_snap&cooldown.frozen_orb.remains>4&(prev_gcd.1.comet_storm|prev_gcd.1.frozen_orb&!talent.comet_storm|cooldown.comet_storm.remains>15&!talent.frostfire_bolt)" );
  actions.aoe_->add_action( "frozen_orb,if=cooldown_react&((!prev_gcd.1.cone_of_cold|!talent.isothermic_core)&(!prev_gcd.1.glacial_spike|!freezable))" );
  actions.aoe_->add_action( "blizzard,if=!prev_gcd.1.glacial_spike|!freezable" );
  actions.aoe_->add_action( "frostbolt,if=buff.icy_veins.up&(buff.deaths_chill.stack<9|buff.deaths_chill.stack=9&!action.frostbolt.in_flight)&buff.icy_veins.remains>8&talent.deaths_chill" );
  actions.aoe_->add_action( "comet_storm,if=!prev_gcd.1.glacial_spike&(!talent.coldest_snap|cooldown.cone_of_cold.ready&cooldown.frozen_orb.remains>20|(cooldown.cone_of_cold.remains>variable.time_to_coc))" );  
  actions.aoe_->add_action( "freeze,if=freezable&debuff.frozen.down&(!talent.glacial_spike|prev_gcd.1.glacial_spike)" );
  actions.aoe_->add_action( "ice_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike)" );
  actions.aoe_->add_action( "frost_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike&!remaining_winters_chill)" );
  actions.aoe_->add_action( "shifting_power,if=cooldown.comet_storm.remains>14" );
  actions.aoe_->add_action( "frostbolt,if=buff.frostfire_empowerment.react&!buff.excess_frost.react&!buff.excess_fire.react" );
  actions.aoe_->add_action( "glacial_spike,if=buff.icicles.react=5&(freezable|(action.flurry.cooldown_react|remaining_winters_chill))" );
  actions.aoe_->add_action( "flurry,if=cooldown_react&!remaining_winters_chill&(buff.brain_freeze.react&!talent.excess_frost|buff.excess_frost.react|prev_gcd.1.glacial_spike)" );
  actions.aoe_->add_action( "ice_lance,if=buff.fingers_of_frost.react|debuff.frozen.remains>travel_time|remaining_winters_chill" );
  actions.aoe_->add_action( "flurry,if=cooldown_react&!remaining_winters_chill" );
  actions.aoe_->add_action( "ice_nova,if=active_enemies>=4&(!talent.glacial_spike|!freezable)&!talent.frostfire_bolt" );
  actions.aoe_->add_action( "cone_of_cold,if=!talent.coldest_snap&active_enemies>=7" );
  actions.aoe_->add_action( "frostbolt" );
  actions.aoe_->add_action( "call_action_list,name=movement" );
    
  actions.cleave_ff_->add_action( "comet_storm,if=prev_gcd.1.flurry|prev_gcd.1.cone_of_cold" );
  actions.cleave_ff_->add_action( "frostbolt,if=buff.icy_veins.up&(buff.deaths_chill.stack<8|buff.deaths_chill.stack=8&!action.frostbolt.in_flight)&buff.icy_veins.remains>8&talent.deaths_chill" );
  actions.cleave_ff_->add_action( "flurry,target_if=min:debuff.winters_chill.stack,if=cooldown_react&(((prev_gcd.1.frostbolt|prev_gcd.1.frostfire_bolt)&buff.icicles.react>=3)|prev_gcd.1.glacial_spike|(buff.icicles.react>=3&buff.icicles.react<5&charges_fractional=2))" );
  actions.cleave_ff_->add_action( "ray_of_frost,target_if=max:debuff.winters_chill.stack,if=remaining_winters_chill=1" );
  actions.cleave_ff_->add_action( "frozen_orb,if=buff.fingers_of_frost.react<2&(!talent.ray_of_frost|cooldown.ray_of_frost.remains)" );
  actions.cleave_ff_->add_action( "cone_of_cold,if=talent.coldest_snap&cooldown.comet_storm.remains>10&cooldown.frozen_orb.remains>10&remaining_winters_chill=0&active_enemies>=3" );
  actions.cleave_ff_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill&buff.icy_veins.down)" );
  actions.cleave_ff_->add_action( "shifting_power,if=cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)|cooldown.icy_veins.remains<20" );
  actions.cleave_ff_->add_action( "glacial_spike,if=buff.icicles.react=5" );
  actions.cleave_ff_->add_action( "frostfire_bolt,target_if=max:debuff.winters_chill.stack,if=buff.frostfire_empowerment.up&remaining_winters_chill" );
  actions.cleave_ff_->add_action( "ice_lance,target_if=max:debuff.winters_chill.stack,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike|remaining_winters_chill&buff.icy_veins.down" );
  actions.cleave_ff_->add_action( "frostbolt" );
  actions.cleave_ff_->add_action( "call_action_list,name=movement" );
    
  actions.cleave_ss_->add_action( "comet_storm,if=prev_gcd.1.flurry&(buff.icy_veins.down)|prev_gcd.1.cone_of_cold|active_enemies>=3" );
  actions.cleave_ss_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&debuff.winters_chill.down&(prev_gcd.1.frostbolt|prev_gcd.1.glacial_spike)" );
  actions.cleave_ss_->add_action( "flurry,target_if=min:debuff.winters_chill.stack,if=cooldown_react&prev_gcd.1.glacial_spike" );
  actions.cleave_ss_->add_action( "freeze,if=freezable&debuff.frozen.down&(!talent.glacial_spike|prev_gcd.1.glacial_spike)" );
  actions.cleave_ss_->add_action( "ice_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike)" );
  actions.cleave_ss_->add_action( "frozen_orb,if=cooldown_react" );
  actions.cleave_ss_->add_action( "shifting_power,if=cooldown.icy_veins.remains>10&cooldown.flurry.remains&(fight_remains>cooldown.icy_veins.remains-6)" );
  actions.cleave_ss_->add_action( "cone_of_cold,if=talent.coldest_snap&cooldown.comet_storm.remains>10&cooldown.frozen_orb.remains>10&remaining_winters_chill=0&active_enemies>=3" );
  actions.cleave_ss_->add_action( "glacial_spike,if=buff.icicles.react=5&((action.flurry.cooldown_react|remaining_winters_chill)|freezable&(cooldown.freeze.ready|cooldown.ice_nova.ready))" );
  actions.cleave_ss_->add_action( "ray_of_frost,if=remaining_winters_chill&buff.icy_veins.down" );
  actions.cleave_ss_->add_action( "frostbolt,if=talent.deaths_chill&(!talent.freezing_rain&buff.icy_veins.remains>8&buff.deaths_chill.stack<=13|talent.freezing_rain&buff.icy_veins.remains>22)" );
  actions.cleave_ss_->add_action( "ice_lance,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike" );
  actions.cleave_ss_->add_action( "frostbolt" );
  actions.cleave_ss_->add_action( "call_action_list,name=movement" );
    
  actions.st_aoebuild_->add_action( "comet_storm,if=prev_gcd.1.flurry" );
  actions.st_aoebuild_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&(buff.icicles.react<5|!talent.frostfire_bolt)&(prev_gcd.1.frostfire_bolt|prev_gcd.1.glacial_spike|prev_gcd.1.frostbolt)|buff.excess_frost.react" );
  actions.st_aoebuild_->add_action( "frozen_orb,if=(!talent.ray_of_frost|cooldown.ray_of_frost.remains>=12&buff.fingers_of_frost.down|talent.splinterstorm)&(buff.icicles.react<5|talent.splinterstorm)" );       
  actions.st_aoebuild_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  actions.st_aoebuild_->add_action( "ray_of_frost,if=remaining_winters_chill" );
  actions.st_aoebuild_->add_action( "shifting_power,if=(buff.icy_veins.down|!talent.deaths_chill)&cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)" );
  actions.st_aoebuild_->add_action( "ice_lance,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike|remaining_winters_chill" );
  actions.st_aoebuild_->add_action( "frostbolt" );
  actions.st_aoebuild_->add_action( "call_action_list,name=movement" );
    
  actions.st_ff_->add_action( "comet_storm,if=prev_gcd.1.flurry|prev_gcd.1.cone_of_cold" );
  actions.st_ff_->add_action( "flurry,if=cooldown_react&buff.icicles.react<5&remaining_winters_chill=0&(debuff.winters_chill.down|buff.brain_freeze.react|buff.excess_frost.react)" );
  actions.st_ff_->add_action( "cone_of_cold,if=talent.coldest_snap&prev_gcd.1.comet_storm&active_enemies>=3" );
  actions.st_ff_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  actions.st_ff_->add_action( "ray_of_frost,if=remaining_winters_chill&(buff.icy_veins.remains<14|buff.spymasters_web.up)" );
  actions.st_ff_->add_action( "frozen_orb" );
  actions.st_ff_->add_action( "shifting_power,if=(buff.icy_veins.down|!talent.deaths_chill)&cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)" );
  actions.st_ff_->add_action( "ice_lance,if=buff.excess_fire.react&remaining_winters_chill=2|remaining_winters_chill=0&debuff.winters_chill.down&buff.fingers_of_frost.react" );
  actions.st_ff_->add_action( "frostbolt" );
  actions.st_ff_->add_action( "call_action_list,name=movement" );
    
  actions.st_ss_->add_action( "comet_storm,if=prev_gcd.1.flurry&buff.icy_veins.down" );
  actions.st_ss_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&debuff.winters_chill.down&(prev_gcd.1.frostbolt|prev_gcd.1.glacial_spike)" );
  actions.st_ss_->add_action( "frozen_orb,if=cooldown_react" );
  actions.st_ss_->add_action( "shifting_power,if=cooldown.icy_veins.remains>10&cooldown.flurry.remains&(fight_remains>cooldown.icy_veins.remains-6)" );
  actions.st_ss_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  actions.st_ss_->add_action( "ray_of_frost,if=remaining_winters_chill&buff.icy_veins.down" );
  actions.st_ss_->add_action( "frostbolt,if=buff.icy_veins.remains>8&buff.deaths_chill.stack<8" );
  actions.st_ss_->add_action( "ice_lance,if=remaining_winters_chill=2|remaining_winters_chill&action.flurry.cooldown_react" );
  actions.st_ss_->add_action( "frostbolt" );
  actions.st_ss_->add_action( "call_action_list,name=movement" );
    
  actions.movement_->add_action( "any_blink,if=movement.distance>10" );
  actions.movement_->add_action( "ice_floes,if=buff.ice_floes.down" );
  actions.movement_->add_action( "ice_nova" );
  actions.movement_->add_action( "cone_of_cold,if=!talent.coldest_snap&active_enemies>=2" );
  actions.movement_->add_action( "arcane_explosion,if=mana.pct>30&active_enemies>=2" );
  actions.movement_->add_action( "fire_blast" );
  actions.movement_->add_action( "ice_lance" );
}
//frost_apl_end
