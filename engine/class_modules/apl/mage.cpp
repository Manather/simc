...
{
  actionpriority_list_t* default = p->get_action_priority_list( "default" );
  action_priority_list_t* precombat = p->get_action_priority_list( "precombat" );
  ...
  precombat_->add_action( "flask" );
  precombat_->add_action( "food" );
  precombat_->add_action( "augmentation" );
  precombat_->add_action( "arcane_intellect" );
  precombat_->add_action( "snapshot_stats" );
  precombat_->add_action( "variable,name=st_aoebuild,value=!talent.deep_shatter&!talent.cold_front" );
  precombat_->add_action( "variable,name=st_ff,value=talent.frostfire_bolt" );
  precombat_->add_action( "variable,name=time_to_coc,value=10*talent.frostfire_bolt" );
  precombat_->add_action( "blizzard,if=active_enemies>=2&talent.ice_caller&!talent.fractured_frost|active_enemies>=4" );
  precombat_->add_action( "frostbolt,if=active_enemies<=3" );

  default_->add_action( "counterspell" );
  default_->add_action( "call_action_list,name=cds" );
  default_->add_action( "run_action_list,name=aoe,if=active_enemies>=7|active_enemies>=3&!talent.frostfire_bolt|active_enemies>=4" );
  default_->add_action( "run_action_list,name=cleave_ff,if=active_enemies>=2&active_enemies<=3&talent.frostfire_bolt" );
  ...
  aoe_->add_action( "frozen_orb,if=cooldown_react&((!prev_gcd.1.cone_of_cold|!talent.isothermic_core)&(!prev_gcd.1.glacial_spike|!freezable))" );
  aoe_->add_action( "blizzard,if=!prev_gcd.1.glacial_spike|!freezable" );
  aoe_->add_action( "frostbolt,if=buff.icy_veins.up&(buff.deaths_chill.stack<9|buff.deaths_chill.stack=9&!action.frostbolt.in_flight)&buff.icy_veins.remains>8&talent.deaths_chill" );
  aoe_->add_action( "comet_storm,if=!prev_gcd.1.glacial_spike&(!talent.coldest_snap|cooldown.cone_of_cold.ready&cooldown.frozen_orb.remains>20|(cooldown.cone_of_cold.remains>variable.time_to_coc))" );
  aoe_->add_action( "freeze,if=freezable&debuff.frozen.down&(!talent.glacial_spike|prev_gcd.1.glacial_spike)" );
  aoe_->add_action( "ice_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike)" );
  aoe_->add_action( "frost_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike&!remaining_winters_chill)" );
  aoe_->add_action( "shifting_power,if=cooldown.comet_storm.remains>14" );
  aoe_->add_action( "frostbolt,if=buff.frostfire_empowerment.react&!buff.excess_frost.react&!buff.excess_fire.react" );
  aoe_->add_action( "glacial_spike,if=buff.icicles.react=5&(freezable|(action.flurry.cooldown_react|remaining_winters_chill))" );
  aoe_->add_action( "flurry,if=cooldown_react&!remaining_winters_chill&(buff.brain_freeze.react&!talent.excess_frost|buff.excess_frost.react|prev_gcd.1.glacial_spike)" );
  aoe_->add_action( "ice_lance,if=buff.fingers_of_frost.react|debuff.frozen.remains>travel_time|remaining_winters_chill" );
  aoe_->add_action( "flurry,if=cooldown_react&!remaining_winters_chill" );
  aoe_->add_action( "ice_nova,if=active_enemies>=4&(!talent.glacial_spike|!freezable)&!talent.frostfire_bolt" );
  aoe_->add_action( "cone_of_cold,if=!talent.coldest_snap&active_enemies>=7" );
  aoe_->add_action( "frostbolt" );
  aoe_->add_action( "call_action_list,name=movement" );

  cds_->add_action( "use_item,name=imperfect_ascendancy_serum,if=buff.icy_veins.remains>19|fight_remains<25" );
  cds_->add_action( "use_item,name=spymasters_web,if=fight_remains<25|(fight_remains<100|buff.spymasters_report.stack>35)&(!talent.deaths_chill&buff.icy_veins.remains>19|talent.deaths_chill&buff.icy_veins.remains>15&buff.icy_veins.remains<20)" );
  cds_->add_action( "potion,if=prev_off_gcd.icy_veins|fight_remains<60" );
  cds_->add_action( "flurry,if=time=0&active_enemies<=2" );
  cds_->add_action( "icy_veins" );
  cds_->add_action( "use_items" );
  cds_->add_action( "invoke_external_buff,name=power_infusion,if=buff.power_infusion.down" );
  cds_->add_action( "invoke_external_buff,name=blessing_of_summer,if=buff.blessing_of_summer.down" );
  cds_->add_action( "blood_fury" );
  cds_->add_action( "berserking" );
  cds_->add_action( "lights_judgment" );
  cds_->add_action( "fireblood" );
  cds_->add_action( "ancestral_call" );

  cleave_ff_->add_action( "comet_storm,if=prev_gcd.1.flurry|prev_gcd.1.cone_of_cold" );
  cleave_ff_->add_action( "frostbolt,if=buff.icy_veins.up&(buff.deaths_chill.stack<8|buff.deaths_chill.stack=8&!action.frostbolt.in_flight)&buff.icy_veins.remains>8&talent.deaths_chill" );
  cleave_ff_->add_action( "flurry,target_if=min:debuff.winters_chill.stack,if=cooldown_react&(((prev_gcd.1.frostbolt|prev_gcd.1.frostfire_bolt)&buff.icicles.react>=3)|prev_gcd.1.glacial_spike|(buff.icicles.react>=3&buff.icicles.react<5&charges_fractional=2))" );
  cleave_ff_->add_action( "ray_of_frost,target_if=max:debuff.winters_chill.stack,if=remaining_winters_chill=1" );
  cleave_ff_->add_action( "frozen_orb,if=buff.fingers_of_frost.react<2&(!talent.ray_of_frost|cooldown.ray_of_frost.remains)" );
  cleave_ff_->add_action( "cone_of_cold,if=talent.coldest_snap&cooldown.comet_storm.remains>10&cooldown.frozen_orb.remains>10&remaining_winters_chill=0&active_enemies>=3" );
  cleave_ff_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill&buff.icy_veins.down)" );
  cleave_ff_->add_action( "shifting_power,if=cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)|cooldown.icy_veins.remains<20" );
  cleave_ff_->add_action( "glacial_spike,if=buff.icicles.react=5" );
  cleave_ff_->add_action( "frostfire_bolt,target_if=max:debuff.winters_chill.stack,if=buff.frostfire_empowerment.up&remaining_winters_chill" );
  cleave_ff_->add_action( "ice_lance,target_if=max:debuff.winters_chill.stack,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike|remaining_winters_chill&buff.icy_veins.down" );
  cleave_ff_->add_action( "frostbolt" );
  cleave_ff_->add_action( "call_action_list,name=movement" );

  cleave_ss_->add_action( "comet_storm,if=prev_gcd.1.flurry&(buff.icy_veins.down)|prev_gcd.1.cone_of_cold|active_enemies>=3" );
  cleave_ss_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&debuff.winters_chill.down&(prev_gcd.1.frostbolt|prev_gcd.1.glacial_spike)" );
  cleave_ss_->add_action( "flurry,target_if=min:debuff.winters_chill.stack,if=cooldown_react&prev_gcd.1.glacial_spike" );
  cleave_ss_->add_action( "freeze,if=freezable&debuff.frozen.down&(!talent.glacial_spike|prev_gcd.1.glacial_spike)" );
  cleave_ss_->add_action( "ice_nova,if=freezable&!prev_off_gcd.freeze&(prev_gcd.1.glacial_spike)" );
  cleave_ss_->add_action( "frozen_orb,if=cooldown_react" );
  cleave_ss_->add_action( "shifting_power,if=cooldown.icy_veins.remains>10&cooldown.flurry.remains&(fight_remains>cooldown.icy_veins.remains-6)" );
  cleave_ss_->add_action( "cone_of_cold,if=talent.coldest_snap&cooldown.comet_storm.remains>10&cooldown.frozen_orb.remains>10&remaining_winters_chill=0&active_enemies>=3" );
  cleave_ss_->add_action( "glacial_spike,if=buff.icicles.react=5&((action.flurry.cooldown_react|remaining_winters_chill)|freezable&(cooldown.freeze.ready|cooldown.ice_nova.ready))" );
  cleave_ss_->add_action( "ray_of_frost,if=remaining_winters_chill&buff.icy_veins.down" );
  cleave_ss_->add_action( "frostbolt,if=talent.deaths_chill&(!talent.freezing_rain&buff.icy_veins.remains>8&buff.deaths_chill.stack<=13|talent.freezing_rain&buff.icy_veins.remains>22)" );
  cleave_ss_->add_action( "ice_lance,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike" );
  cleave_ss_->add_action( "frostbolt" );
  cleave_ss_->add_action( "call_action_list,name=movement" );
  
  st_aoebuild_->add_action( "comet_storm,if=prev_gcd.1.flurry" );
  st_aoebuild_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&(buff.icicles.react<5|!talent.frostfire_bolt)&(prev_gcd.1.frostfire_bolt|prev_gcd.1.glacial_spike|prev_gcd.1.frostbolt)|buff.excess_frost.react" );
  st_aoebuild_->add_action( "frozen_orb,if=(!talent.ray_of_frost|cooldown.ray_of_frost.remains>=12&buff.fingers_of_frost.down|talent.splinterstorm)&(buff.icicles.react<5|talent.splinterstorm)" );
  st_aoebuild_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  st_aoebuild_->add_action( "ray_of_frost,if=remaining_winters_chill" );
  st_aoebuild_->add_action( "shifting_power,if=(buff.icy_veins.down|!talent.deaths_chill)&cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)" );
  st_aoebuild_->add_action( "ice_lance,if=buff.fingers_of_frost.react&!prev_gcd.1.glacial_spike|remaining_winters_chill" );
  st_aoebuild_->add_action( "frostbolt" );
  st_aoebuild_->add_action( "call_action_list,name=movement" );
  ...
  st_ff_->add_action( "comet_storm,if=prev_gcd.1.flurry|prev_gcd.1.cone_of_cold" );
  st_ff_->add_action( "flurry,if=cooldown_react&buff.icicles.react<5&remaining_winters_chill=0&(debuff.winters_chill.down|buff.brain_freeze.react|buff.excess_frost.react)" );
  st_ff_->add_action( "cone_of_cold,if=talent.coldest_snap&prev_gcd.1.comet_storm&active_enemies>=3" );
  st_ff_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  st_ff_->add_action( "ray_of_frost,if=remaining_winters_chill&(buff.icy_veins.remains<14|buff.spymasters_web.up)" );
  st_ff_->add_action( "frozen_orb" );
  st_ff_->add_action( "shifting_power,if=(buff.icy_veins.down|!talent.deaths_chill)&cooldown.frozen_orb.remains>10&(!talent.comet_storm|cooldown.comet_storm.remains>10)&(!talent.ray_of_frost|cooldown.ray_of_frost.remains>10)" );
  st_ff_->add_action( "ice_lance,if=buff.excess_fire.react&remaining_winters_chill=2|remaining_winters_chill=0&debuff.winters_chill.down&buff.fingers_of_frost.react" );
  st_ff_->add_action( "frostbolt" );
  st_ff_->add_action( "call_action_list,name=movement" );

  st_ss_->add_action( "comet_storm,if=prev_gcd.1.flurry&buff.icy_veins.down" );
  st_ss_->add_action( "flurry,if=cooldown_react&remaining_winters_chill=0&debuff.winters_chill.down&(prev_gcd.1.frostbolt|prev_gcd.1.glacial_spike)" );
  st_ss_->add_action( "frozen_orb,if=cooldown_react" );
  st_ss_->add_action( "shifting_power,if=cooldown.icy_veins.remains>10&cooldown.flurry.remains&(fight_remains>cooldown.icy_veins.remains-6)" );
  st_ss_->add_action( "glacial_spike,if=buff.icicles.react=5&(action.flurry.cooldown_react|remaining_winters_chill)" );
  st_ss_->add_action( "ray_of_frost,if=remaining_winters_chill&buff.icy_veins.down" );
  st_ss_->add_action( "frostbolt,if=buff.icy_veins.remains>8&buff.deaths_chill.stack<8" );
  st_ss_->add_action( "ice_lance,if=remaining_winters_chill=2|remaining_winters_chill&action.flurry.cooldown_react" );
  st_ss_->add_action( "frostbolt" );
  st_ss_->add_action( "call_action_list,name=movement" );
}
//frost_apl_end
